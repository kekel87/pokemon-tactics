...
### Pattern manquant : Arc frontal (slash)

Plusieurs attaques evoquent un **mouvement de balayage** qui touche les 3 cases devant le lanceur (face + 2 diagonales adjacentes). Exemples : Tranch'Herbe, Aile d'Acier, Queue de Fer, Coupe.

**Proposition** : ajouter un pattern `slash` ou `arc` avec un parametre `width` (nombre de cases en arc).
...
### 6.1 Cleave / Arc frontal

**Description** : touche les 3 cases devant (face + 2 diagonales adjacentes a la case en face)
**Parametres** : `range` (1 = adjacent, 2 = une case plus loin ?)
**Candidats roster actuel** : Tranch'Herbe, (Cru-Aile ?)
**Candidats futurs** : Aile d'Acier, Queue de Fer, Coupe, Griffe Ombre, Lame de Roc

```
    D . D       (D = case touchee, diagonales)
...
**Candidats** : Bombe-Beurk (r2), Ball'Ombre (r1), Bombe Oeuf, (Tornade ?)
...
| 4 | Bombe-Beurk | Sludge Bomb | cross 2-4 s3 | **blast 2-4 r2** | CHANGER |
