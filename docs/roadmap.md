...
- [ ] **Mécaniques restantes — moves « complexes » (nouvelle méca par type)** — ~159 moves restants learnable par le roster, chacun demande une mécanique non encore codée. Une entrée par type, attaquée 1 par 1. (Plusieurs déjà listées plus haut : Champs/Terrains, Trick Room, Contrôle moves, Delayed/countdown, Hazards.)
  ...
  - [ ] **Secondaire conditionnel si cible boostée (~2)** — `alluring-voice` (Confusion si cible a eu un boost ce tour), `burning-jealousy` (Brûlure si cible a eu un boost ce tour). Nécessite tracking `wasStatBoostedThisTurn` sur `PokemonInstance`.
