...
- [ ] **Mécaniques avancées restantes** — sous-plans à attaquer après Team Builder. Doc détaillée écrite au moment d'attaquer chaque plan.
  ...
  - [ ] **TP moves (`TargetingKind.Teleport`)** — plan 088. Nouvelle catégorie targeting : cible libre dans range, **LoS/terrain/hauteur bypassés**, caster TP sur tile target, dégâts si occupée, atterrissage = cardinale aléatoire (BFS fallback). Terrains dangereux **inclus** = contrepartie risque vs charge classique. 7 moves : teleport (pur), fly, dig, bounce, phantom-force, shadow-force, dive. Décisions actées 2026-05-20.
  - [ ] **TP inversé (`TargetingKind.HitAndRun`)** — plan dédié. Caster frappe Single, puis choisit zone recul TP, atterrit dans zone choisie. Moves : u-turn, volt-switch, flip-turn.
