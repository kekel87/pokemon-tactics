...
- [ ] **Mécaniques restantes — moves « complexes » (nouvelle méca par type)** — **~160 moves « complexes »** (185 restants audit 2026-06-02, −12 livrés plan 109, −2 livrés plan 110 → ~171, dont ~9 familles power conditionnel encore à faire). Chacun demande une mécanique non encore codée. Une entrée par type, attaquée 1 par 1. Pool learnable roster total = 494 moves ; 323 implémentés in-pool + 21 hors-pool.
  ...
  - [ ] **Power conditionnel — familles restantes (~20)** — Poids : Lancer Gravé/`low-kick` + Écras'Herbe/`grass-knot` + Lourde Bombe/`heavy-slam` + Dévastateur/`heat-crash` (besoin `weightkg` sur instance). Timing : Avalanche/`avalanche` + Représailles/`payback` + Vendetta/`revenge` + Bégaiement/`stomping-tantrum` + Durement/`retaliate` (×2 si frappé avant ou conditions timing). Compteurs : Poing Rageur/`rage-fist` + Mélodie Écho/`echoed-voice` + Unisson/`round` + Dernier Recours/`last-resort`. Terrain : Tension Montante/`rising-voltage` + Force Amplifiée/`expanding-force` + Impulsion Terrain/`terrain-pulse` (après plan Champs/Terrains). Divers : Dernier Tribut/`last-respects`, Nage Furieuse/`fishious-rend` + Bec Supersonique/`bolt-beak`.
