...
- [ ] **Mécaniques avancées restantes** — sous-plans à attaquer après Team Builder. Doc détaillée écrite au moment d'attaquer chaque plan.
  ...
  - [x] **Barrières aura mobile — plan 095 terminé 2026-05-23.** Reflect / Light Screen, aura mobile r3 Manhattan suit caster. Casse-Brique interaction. Aurora Veil reporté (0 learner Gen 1).
  ...
  - [x] **TP moves (`TargetingKind.Teleport`) — plan 088 terminé 2026-05-21.** Cible libre dans range, LoS/terrain/hauteur bypassés, caster TP sur tile target, dégâts si occupée, atterrissage cardinal (BFS fallback). 7 moves livrés : teleport, fly, dig, bounce, phantom-force, shadow-force, dive. États semi-invulnérables Flying/Burrowing/Diving/Vanished. Décisions actées 2026-05-20.
  - [x] **TP inversé (`TargetingKind.HitAndRun`) — plan 092 terminé 2026-05-21.** Caster frappe Single, puis choisit tile retraite dans `retreatRange` Chebyshev. Miss/Protect bloque retraite (parité Showdown). Const enum `HitAndRunRetreatFallbackReason`. IA greedy max-distance. Moves : `u-turn` (Insecte 70BP contact), `volt-switch` (Élec 70BP non-contact hitRange 1-2), `flip-turn` (Eau 60BP contact). **157 moves** total.
  - [x] **MoveCharging visible — plan 094 terminé 2026-05-22.** skull-bash / sky-attack / razor-wind. Indicateur ⚡, Flinch volatile, `MoveDefinition.chargeEffects`. **160 moves** total.
  - [x] **Baton Pass — plan 093 terminé 2026-05-22.** `EffectKind.TransferStatStages`, ciblage allié r1, reset caster. Volatils non transférés.
  - [x] **Substitute — plan 099 terminé 2026-05-28.** Pure field `PokemonInstance.substituteHp?`, `EffectKind.PostSubstitute`, damage routing, block statuts/baisses-stats, flags sound/bypasssub, 4 BattleEventTypes, overlay sprite `dummy`, InfoPanel badge, i18n 17 clés, IA scoring, 7 OP sets sub variants (163→169 sets), 10 tests intégration. **165 moves** total.
  - [x] **Taunt — plan 100 terminé 2026-05-28.** `StatusType.Taunted` volatile 3 tours, check `executeUseMove` + `getLegalActions`, `taunt-tick-handler.ts`, interactions canon (Sub bloque, Safeguard/Mist ne bloquent pas), 4 OP sets taunt (169→173 sets), InfoPanel badge + MoveTooltip tag rouge + floating text + BattleLogFormatter, IA `scoreTauntApplication`. **166 moves** total.
  - [ ] **Contrôle moves** (Disable / Encore / Torment / Heal Block / Imprison / Magic Coat) — statuts volatils modifiant `getLegalActions`. Disable 4 tours (1 move grisé), Encore 3 tours (forcé répéter), Torment 5 tours (pas répéter même move), Heal Block 5 tours, Imprison tant que lanceur vivant, Magic Coat single-turn reflect.
...
- [ ] **Content Batches G — moves « simples » (shape + effet déjà supporté)** — ~160 moves Gen 1 learnable par le roster, sans nouvelle mécanique (cf. analyse 2026-05-31). Découpés en 5 batches faits 1 par 1. Chaque move = entrée `tactical.ts` (targeting + effects) + i18n FR/EN + pattern (`move-pattern-designer`, à décider au lancement de chaque batch). Pas de sprite, OP sets optionnels.
  - [ ] **G1 — Damage pur physique (40)** : `throat-chop, trailblaze, mega-kick, brutal-swing, high-horsepower, seed-bomb, superpower, smart-strike, x-scissor, lash-out, flame-charge, ice-spinner, fire-fang, temper-flare, steel-wing, supercell-slam, thunder-fang, peck, aqua-jet, megahorn, power-whip, ice-fang, fury-cutter, hammer-arm, ice-shard, pay-day, slam, psychic-fangs, metal-claw, horn-attack, poltergeist, bullet-punch, fell-stinger, mach-punch, shadow-punch, vine-whip, cut, raging-bull, meteor-mash, steel-roller`
  - [ ] **G2 — Damage pur spécial + multi-hit (28)** : spécial — `swift, psyshock, dragon-pulse, dazzling-gleam, hyper-voice, alluring-voice, overheat, vacuum-wave, leaf-storm, aura-sphere, burning-jealousy, magical-leaf, power-gem, disarming-voice, draco-meteor, shock-wave, psystrike` ; multi-hit (`effect.hits`, déjà supporté) — `dual-wingbeat, rock-blast, scale-shot, bullet-seed, double-hit, triple-axel, pin-missile, fury-attack, bone-rush, icicle-crash, tail-slap`
  - [ ] **G3 — Damage + statut/flinch/confusion secondaire (27)** : `snore, zen-headbutt, poison-jab, water-pulse, heat-wave, scorching-sands, iron-head, dark-pulse, hurricane, gunk-shot, scald, thunder-shock, cross-poison, zap-cannon, dragon-rush, inferno, spark, astonish, powder-snow, sludge, smog, extrasensory, nuzzle, axe-kick, poison-tail, twister, freeze-dry`
  - [ ] **G4 — Damage + stat-drop / high-crit / recoil / drain (36)** : `take-down, bulldoze, rock-tomb, stone-edge, chilling-water, mud-shot, drain-punch, mud-slap, liquidation, earth-power, wild-charge, drill-run, muddy-water, shadow-claw, low-sweep, air-cutter, acid-spray, lunge, electroweb, pounce, skitter-smack, play-rough, brave-bird, draining-kiss, breaking-swipe, absorb, psycho-cut, bug-buzz, snarl, razor-shell, mystical-fire, struggle-bug, night-slash, wood-hammer, crush-claw, wave-crash`
  - [ ] **G5 — Pure stat + pure statut (27)** : stat (StatChange) — `scary-face, charm, leer, fake-tears, spite, eerie-impulse, tail-whip, tickle, metal-sound, coaching, harden, baby-doll-eyes, work-up, confide, sweet-scent, venom-drench, string-shot, magnetic-flux, howl, rock-polish` ; statut — `poison-powder, stun-spore, poison-gas, sweet-kiss, swagger, flatter, teeter-dance`

- [ ] **Mécaniques restantes — moves « complexes » (nouvelle méca par type)** — ~159 moves restants learnable par le roster, chacun demande une mécanique non encore codée. Une entrée par type, attaquée 1 par 1. (Plusieurs déjà listées plus haut : Champs/Terrains, Trick Room, Contrôle moves, Delayed/countdown, Hazards.)
  - [ ] **Power conditionnel (~34)** — BP dépend d'un état runtime. `facade` (×2 si statut), `hex`/`venoshock` (×2 si cible statut), `payback`/`avalanche`/`revenge` (×2 si frappé avant), `body-press` (utilise Def), `foul-play` (Atk cible), `acrobatics` (×2 sans objet), `stored-power`/`power-trip` (+20/stage), `electro-ball`/`gyro-ball` (ratio vitesse), `low-kick`/`grass-knot`/`heavy-slam`/`heat-crash` (poids), `flail`/`reversal` (HP bas), `brine` (×2 cible <50%), `weather-ball`/`terrain-pulse` (météo/champ), `rising-voltage`/`expanding-force` (terrain), `last-resort`, `retaliate`, `round`, `echoed-voice`, `hard-press`, `last-respects`, `rage-fist`, `stomping-tantrum`, `fishious-rend`/`bolt-beak`, `water-spout`/`eruption` (HP%).
  ...
  - [ ] **Item interaction (~8)** — manipulation objet tenu. `knock-off` (×1.5 + retire objet), `thief`/`covet` (vole), `trick`/`switcheroo` (échange), `fling` (lance objet), `pluck`/`bug-bite` (mange baie), `incinerate` (brûle baie), `corrosive-gas`. Prérequis : système objets consommables/retirables en combat.
  ...
  - [ ] **Type / transform manip (~5)** — `transform` (copie cible : stats/moves/type), `conversion`/`conversion-2`, `reflect-type`, `soak` (change type cible). Lourd (mutation runtime du Pokemon).
  - [ ] **Move-copy (~6)** — appelle/copie un autre move. `metronome` (random), `mimic`/`sketch`, `mirror-move`/`copycat` (dernier move), `nature-power` (selon terrain), `sleep-talk` (random parmi les siens, pendant sommeil). Réentrance moteur à gérer.
  ...
  - [ ] **Crash on miss (~1)** — `jump-kick`/`high-jump-kick` (dégâts self si miss/immune).
  - [ ] **Misc volatile / utility (~9)** — `curse` (Ghost self-KO partiel vs non-Ghost stat), `yawn` (sleep T+1), `focus-energy`/`laser-focus` (crit↑), `magnet-rise` (lévite 5t), `ingrain`, `follow-me`/`rage-powder`/`spotlight` (redirect ciblage — pertinent en grid ?), `after-you`/`quash` (réordonne CT), `ally-switch` (échange position alliés), `helping-hand` (déjà Delayed), `beat-up`, `belly-drum` (-50% HP +6 Atk), `acupressure` (+2 stat random), `false-swipe`/`feint` (laisse 1 HP / casse Protect), `smack-down` (ground), `worry-seed`/`gastro-acid`/`role-play`/`skill-swap` (manip ability), `pursuit` (×2 si cible « fuit »).
