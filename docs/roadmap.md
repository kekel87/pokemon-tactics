...
- [ ] **Mécaniques avancées restantes** — sous-plans à attaquer après Team Builder. Doc détaillée écrite au moment d'attaquer chaque plan.
  ...
  - [ ] **MoveCharging visible** — plan dédié pour Skull Bash / Sky Attack / Razor Wind. Stay-in-place T1 (boost stat/charge), frappe T2 en ligne. Solar-Beam fournit squelette.
  - [ ] **Baton Pass** (mécanique spéciale, hors TP) — `EffectKind.TransferStatStages` ciblage allié adjacent r1. Passe stat stages courants au receveur, reset caster. Question scope (statuts volatils inclus ou stages only) à trancher au plan.
