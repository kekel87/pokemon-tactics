...
- [ ] **Mécaniques restantes — moves « complexes » (nouvelle méca par type)** — **~87 moves « complexes »** restants (493 pool − 406 implémentés in-pool après Contrôle + Hazards). Chacun demande une mécanique non encore codée. Une entrée par type, attaquée 1 par 1. +21 hors-pool (signatures).
  ...
  - [ ] **Misc volatile / utility (~26, découpé en batches A→E, validé humain 2026-07-05)**
    ...
    - [ ] **Batch B — dégâts utilitaires** (à préciser) — Faux-Change/`false-swipe` (laisse 1 HP) + Ruse/`feint` (casse Protect), Anti-Air/`smack-down` (ground), Poursuite/`pursuit` (×2 si cible « fuit »), Croc Fatal/`super-fang` (dmg = 1/2 HP cible), Baston/`beat-up`, **Corps Perdu/`vital-throw`** (never-miss + priorité -1 « frappe en dernier »).
    - [ ] **Batch C — manip talent** (à préciser) — Soucigraine/`worry-seed` + Suc Digestif/`gastro-acid` + Imitation/`role-play` + Échange/`skill-swap`.
    - [ ] **Batch D — buff/statut** (à préciser) — Malédiction/`curse` (Ghost self-KO partiel vs non-Ghost stat), Bâillement/`yawn` (sleep T+1), Cognobidon/`belly-drum` (-50% HP +6 Atq), Acupression/`acupressure` (+2 stat random), Attraction/`attract` (volatile infatuation), Vol Magnétik/`magnet-rise` (lévite 5t).
    - [ ] **Batch E — grille-problématiques** (à préciser) — Par Ici/`follow-me` + Poudre Fureur/`rage-powder` (redirect ciblage — pertinent en grid ?), Après Vous/`after-you` (réordonne CT), Interversion/`ally-switch` (échange position alliés). Coup d'Main/`helping-hand` déjà livré (plan 133).

- [ ] **Talents & items de finition** — compléter pool transversal une fois moves livrés. Pool restant : abilities 95/114 (Tiers A/B/C livrés — plans 137-138, v2026.6.4), items 92/~159 (50 objets ajoutés — v2026.6.4). OP sets déjà à 100% — finition = couverture exhaustive movepools Gen 1 pour Team Builder libre.
