...
- [ ] **Content Batches G — moves « simples » (shape + effet déjà supporté)** — moves Gen 1 learnable par le roster, sans nouvelle mécanique (cf. analyse 2026-05-31, audit complet 2026-06-02). Découpés en batches faits 1 par 1. Chaque move = entrée `tactical.ts` (targeting + effects) + i18n FR/EN + pattern (`move-pattern-designer`, à décider au lancement de chaque batch). Pas de sprite, OP sets optionnels.
  ...
  - [ ] **G6 — Simples oubliés des batches G1-G5 (11)** — audit complet 2026-06-02 a révélé 11 moves « simples » non couverts (méca déjà supportée). Force/strength (dmg pur), Écrasement/stomp (dmg + flinch 30%), Double Baffe/dual-chop (multi-hit ×2), Camaraderie/play-nice (debuff Atq -1 single), Rafale Feu/blast-burn + Végé-Attaque/frenzy-plant + Giga Impact/giga-impact + Hydroblast/hydro-cannon (recharge — flag `move.recharge` réutilisable confirmé), Lame Solaire/solar-blade (charge 2T + sun-skip), Laser Météore/meteor-beam (charge 2T + chargeEffects AtqSpé+1), Trempette/splash (no-op assumé « pour le lol »).

- [ ] **Mécaniques restantes — moves « complexes » (nouvelle méca par type)** — **~174 moves « complexes »** (audit 2026-06-02 : 185 restants learnable au total, dont 11 simples → Batch G6 ci-dessus). Chacun demande une mécanique non encore codée. Une entrée par type, attaquée 1 par 1. Inclut les *deferred* des batches G (cf. tags *Deferred batch Gx* ci-dessous). Pool learnable roster total = 494 moves ; 309 implémentés in-pool + 12 hors-pool.
