...
- [ ] **Mécaniques restantes — moves « complexes » (nouvelle méca par type)** — **~100 moves « complexes »** restants (493 pool − 393 implémentés in-pool). Chacun demande une mécanique non encore codée. Une entrée par type, attaquée 1 par 1. +21 hors-pool (signatures).
  ...
  - [ ] **Power conditionnel — familles restantes (~7)** — ~~Poids (plan 111)~~ ✅ · ~~Timing/Compteurs/Terrain livrés B3 (plan 115)~~ ✅. Divers restants : Dernier Tribut/`last-respects`, Nage Furieuse/`fishious-rend` + Bec Supersonique/`bolt-beak`.
...
- [x] **Champs (Herbeux, Électrique, Brumeux, Psychique) — plans 117 + 118 terminés 2026-06-08/09.** Livré en Phase 4. Système `FieldTerrain` zones Manhattan r3, multi-zones coexistantes, 4 poseurs + 7 moves dépendants (B4 10/10). Distorsion (Trick Room) différée — reste dans la liste des mécaniques complexes Phase 4.
