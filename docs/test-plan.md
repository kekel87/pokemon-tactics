...
- 🤖 **Anim de repos Volant selon le terrain d'atterrissage** : un Volant qui termine un déplacement
  sur un terrain **fly-over** (sans sol — eau/lave/marais/sable/neige/**glace**/obstacle) **reste en
  vol** (`restingAnimation` = candidat glide, ici « FlyingIdle ») ; sur un **sol praticable**
  (normal) il **se pose** (`restingAnimation` = « Idle »). Verrouillé via le hook `spriteStates`
  après un déplacement 1-case piloté (`flying-resting-anim.spec` ; posé sur sol, vol sur marais +
  glace). *Le glide visuel au spawn = 👁 ci-dessus ; la table `isFlyoverTerrain` = unit view-core.*
...
- 🤖 Liquide peu profond (**eau/marais**) : fond opaque `tile_x_y` (groupe 0, pickable) **+** nappe
  translucide séparée `liquid_surface_x_y` (groupe sprite 2, `transparent`) — `scene-graph.spec`.
  *La texture de fond (sable/roche), l'opacité exacte par liquide, la teinte = 👁.*
- 🤖 Liquide plein (**lave / eau profonde**) : UNE seule colonne `tile_x_y` 0→5/6, **sans** nappe
  `liquid_surface_x_y` séparée (le split fond+surface montrerait une couture bicolore à 3/6). La lave
  a rejoint l'eau profonde sur la branche colonne unique — `scene-graph.spec`.
- 👁 **Anim procédurale des liquides** (lueur / scintillement / ondulation via `LiquidShimmerPlugin`)
  sur les matériaux de surface liquide : effet shader continu, non exposé par le hook scène → SENS
  non automatisable.
...
- 👁 **Eau profonde** : colonne translucide **sans fond visible** (pas de sable au fond, contrairement
  à l'eau peu profonde/marais). La **lave** rend une colonne pleine (corps de magma opaque).
...
- 🤖 **La bonne feuille de tuiles est chargée selon le pointeur** (plan 185) : `input-prompts-pixel-1-bit`
  (souris) en pointeur fin, `cursor-pixel-pack` (main de tap) en pointeur grossier — les deux
  feuilles n'ont ni la même géométrie ni la même URL, un consommateur doit changer feuille et grille
  ensemble (`input-prompt-glyph.spec`, `hasTouch`).
- 👁 **Dessin** du glyphe : souris en pointeur fin, main en pointeur grossier (tuile posée en masque
  CSS, teinte héritée de la pastille, `image-rendering: pixelated`). Choix de feuille fait en CSS pur
  (`@media (pointer: coarse)`), donc rebrancher une souris sur un téléphone re-style la pastille sans
  re-rendu — c'est du pixel, invisible au DOM.
...
### 4.18 Contrôles tactiles + légende de contrôles (plan 183 Lot 1, plan 185)

*src : `render-babylon/combat-scene.ts` (table de pointeurs, pinch, sélecteur de direction),
`render-babylon/babylon-compass.ts` (boussole tapable, sans glyphe — mesh anneau supprimé au
plan 185), `view-core/battle-orchestrator.ts` (visée d'un pattern directionnel + annulation),
`ui-dom/battle-chrome.ts`, `ui-dom/chrome-insets.ts` (mesure de la timeline, dotée d'un
`subscribe` — lue par le renderer ET par la légende), `ui-dom/control-legend.ts` +
`styles/control-legend.css` (légende DOM), `app/src/input/key-legend.ts` (capuchon de touche
selon la disposition clavier)*
*e2e : `combat/touch-controls.spec.ts`, `combat/compass-and-legend.spec.ts` (ex
`compass-rotate-hint.spec.ts`, renommé et étendu à la légende au plan 185)*
...
- 🤖 **Boussole tapable** : une pression sur elle donne **un cran de rotation** de la vue. Vérifié par
  une vraie pression souris (`page.mouse`) sur le point où la scène dessine le proxy de picking,
  donc à travers la même couche d'entrée que le tap (`compass-and-legend.spec`). *Le tap au doigt
  reste 👁 (téléphone réel, 2026-08-20).*
- 🤖 **Ancrage repris de la première case de la timeline** : la zone tapable commence juste après le
  portrait actif (écart de dégagement de 6 px, donc sans le recouvrir), à n'importe quelle taille
  d'écran (`compass-and-legend.spec`). Trois tentatives à base de constantes maison avaient dérivé :
  derrière la timeline, puis flottante, puis glissant à chaque changement de taille — la mesure à
  l'exécution (`chrome-insets.ts`) est désormais observée, et **lue par deux consommateurs** (le
  renderer ET la légende de contrôles, plan 185 — une mesure, un propriétaire, décision #798). *La
  **taille** de l'aiguille reste 👁 : son mesh garde une boîte englobante non recentrée, donc sa
  projection ne la mesure pas.*
- 🤖 **Zone tapable CARRÉE, plancher 44 px** : le glyphe de rotation qui vivait dans la scène Babylon
  à droite de la boussole a été **supprimé** (plan 185, remplacé par la ligne rotation de la légende
  DOM ci-dessous) — le proxy de picking n'a donc plus de raison de s'étendre vers la droite pour
  l'englober, il redevient un simple carré ancré sur le portrait, mesuré par sa **projection à
  l'écran** (hook `meshScreenBox`) (`compass-and-legend.spec`).
...
- 🤖 **La légende reste immobile quand la timeline perd son entrée active** (case vide pendant la
  prévisualisation de coût en CT) : deux tentatives antérieures — ancrage DOM dans la case active,
...
- 🤖 **Résolution de la touche affichée selon la disposition clavier** (`getLayoutMap` +
  repli par langue du jeu, `KeyQ` → `A` en AZERTY / `Q` en QWERTY) : **unit**
  (`app/input/key-legend.test.ts`, 6 cas — API absente/refusée, `SecurityError`, FR/EN), pas e2e
  (`navigator.keyboard` n'est pas instrumentable par Playwright).
...
- 🤖 **§5.20 objets « eject » (lot 102→104)** — téléportation forcée vers la zone de spawn quand le
  ...
  - 🤖 **Orbe Vie** (`life-orb`) — recul 1/10 PV max au PORTEUR, **UNE SEULE fois par attaque**, y
    compris sur un move multi-coups (fix : le hook attaquant `onAfterMoveDamageDealt`, émetteur du
    HeldItemActivated, est appelé une seule fois en fin de `handleDamage` sur le TOTAL des dégâts, plus
    par coup). Régression avant le fix : le recul frappait PAR coup → l'Orbe s'annonçait 2 à 5 fois sur
    Balle Graine. Piloté de bout en bout (`mechanics-items.spec`) : le joueur Florizarre tient l'Orbe
    (`heldItem`) et lance Balle Graine (`bullet-seed`, 2-5 coups, 100 %) sur le dummy endurant
    (`dummyHp: 999`, survit à la volée) → le journal ne compte **qu'une** ligne « Orbe Vie de <X>
    s'active ! » (`toHaveCount(1)`), quel que soit le nombre de coups tiré par le seed — ce qui
    distingue le fix (1) de la régression (≥ 2). *Le MONTANT chiffré du recul (fait de PV, non exposé
    au journal/scène) et le fire-once sur move mono-coup sont couverts integration core
    (`held-items.integration.test.ts` : « recoil UNE seule fois »).* **Canon à distinguer du Casque
    Brut** (`rocky-helmet`) — objet DÉFENSEUR dont le recoil de contact frappe l'attaquant **PAR coup**
    (correct : une réaction par contact reçu, cf témoins Pare-Effet/Gant de Boxe §5.17/§5.18).
  - 🤖 **Casque Brut — coup fantôme (régression)** (`rocky-helmet`) — sur un move contact
    **multi-coups**, le recoil de contact frappe l'attaquant PAR coup ; si l'attaquant est à très bas
    PV, le recul du **coup 1** le met K.O. **avant** le coup 2. Régression avant le fix : la boucle
    multi-coups continuait quand même au coup 2 (« un mort frappait encore » → 2 coups sur la cible).
    Fix core : `if attacker.currentHp <= 0 break` en tête de boucle (`battle/handlers/handle-damage.ts`)
    → la volée s'arrête au K.O. de l'attaquant. Piloté de bout en bout (`mechanics-items.spec`) : le
    joueur Florizarre (`hp: 1` = 1 % PV max → le recul du Casque le K.O. dès le 1er coup) lance Double
    Pied (`double-kick`, 2 coups FIXES, contact, 100 %) sur le dummy Ronflex porteur du Casque Brut
    (`dummyHeldItem`) adjacent → « Florizarre est K.O. ! » (recul mortel du 1er coup) + le récap
    multi-coups « **Touché 1 fois !** » (et JAMAIS « Touché 2 fois ! » — la signature du bug). Le récap
    encode EXACTEMENT le nombre de coups → signal **contamination-proof** (immunisé des ticks de terrain
    de fin de tour, contrairement au comptage de « perd N PV »). *Le montant chiffré du recul et le
    fire-once sont couverts integration core (`held-items.integration.test.ts` : « pas de coup
    fantôme »).*
...
- 🤖 **Portée dynamique selon la hauteur** : `getHeightRangeBonus = clamp(0, 2, floor(Δh/2))` (positif
  seulement) allonge la portée des moves à distance vers les cases en contrebas (Single portée>1,
  Blast, Ligne, Cône ; mêlée/Dash/Tranche exclus). Sur `sandbox-fall-4` (plateau h5 / fosse h1), une
  cible à **distance 3** — hors de la portée de base 1-2 d'Aéropique — devient ATTAQUABLE depuis le
  plateau (Δh=4 → +2 → portée 1-4) : la frappe résout (« utilise Aéropique »), alors qu'à plat (même
  distance 3, Δh=0) elle reste hors de portée (aucune résolution) — `height.spec`. *Le montant du
  bonus (5 cas), l'exclusion mêlée/Dash/Tranche et l'application par pattern (resolveSingle/Blast/
  Cone/Line + getValidTargetPositions) = SENS unit/integration core (`battle/height-modifier.test.ts`,
  `grid/targeting.integration.test.ts`).*
...
- 🤖 **Boul'Armure ×2 Roulade** (`defense-curl` → `rollout`) : Boul'Armure pose le flag persistant
  `usedDefenseCurl` sur le lanceur → Roulade (et Ball'Glace) inflige le DOUBLE de puissance
  (30 → 60 BP à l'index 1). Deux Roulade « fraîches » sur `sandbox-flat` (Rhinoféros vs Ronflex
  endurant, mêmes espèces/positions/seed) : sans Boul'Armure vs après Boul'Armure (qui remet le streak
  à 0 → même index de streak) → les PV retirés (barre de vie InfoPanel) au moins **×1,6** après
  Boul'Armure (`mechanics-dynamic-power.spec`). *Le facteur ×2 exact, le reset au K.O. et Ball'Glace =
  SENS unit/integration core (`moves/{defense-curl,rollout}.test.ts`, `dynamic-power-system`).*
...
- 🤖 **Zone Magique — objet neutralisé (Ballon)** : adoption canonique complète du chokepoint
  `effectiveHeldItem` (plan 171) — un mon debout DANS la Zone Magique voit TOUS les effets de son objet
  tenu neutralisés. Signal e2e le plus net : le **Ballon** (`air-balloon`, immunité au type Sol). Le
  Mackogneur pose Zone Magique (2,3) puis lance **Séisme** (Sol) ; le Ronflex adjacent porteur du Ballon
  est dans le diamant → son immunité Sol tombe → journal « Ronflex perd N PV ! » ; le témoin (même duel
  SANS Zone Magique) reste immunisé (aucune ligne de dégâts) — `mechanics-field-global.spec`.
- 👁 **Zone Magique — autres objets neutralisés** (objet inerte mais NON consommé, redevient actif hors
  zone) : les 15 autres effets d'objet routés via `effectiveHeldItem` restent couverts unit+intégration
  core — injouables ou non-binaires en 1v1 self-cast. Injouables : **Veste de Combat** (`assault-vest`,
  `forbidsStatusMoves`) — son porteur ne peut pas lancer Zone Magique (un move statut) ; **Herbe Mentale**
  (`mental-herb`) / verrou **Choice** (`onMoveLock`) — négatifs multi-tour au ciblage fin. Non-binaires
  au journal : **Dé Pipé** (`loaded-dice`, nombre de coups), **Grosse Racine** (`big-root`, magnitude de
  soin), **Talisman Sain** (`clear-amulet`), **Lunettes Filtre** (`safety-goggles`, immunité poudre —
  la Zone AoE de la poudre touche aussi le lanceur → assertion négative ambiguë), Roche Royale/Croc
  Rasoir (`onFlinchChance`), Spray Gorge (`onAfterMoveUse`), **Lancer/Dégommage** (`fling`, échoue sous
  zone), bloc terrain (`onTerrainTick`). Intégration core `moves/magic-room.test.ts` pilote un vrai
  combat pour les objets suppressibles historiques — Cape Obscure (`covert-cloak`), Carapace Mue
  (`shed-shell`), Accro Griffe (`grip-claw`), Bande Étreinte (`binding-band`), Pierrallégée
  (`float-stone`). Carve-outs canon NON neutralisés (décision 2026-07-24) : baies de manipulation —
  Calcination (`incinerate`) détruit la baie, Picore/Piqûre (`bug-bite`/`pluck`) mange la baie ennemie ;
  + helpers stat purs Pierrallégée/Poudre Vite (`quick-powder`, NON suppressible, décision #714).
...
- 🤖 **« ⏳ Place libre »** pour une place que personne ne tient — et **pas** « IA », qui faisait
  ...
  `start`.
- 🔴 🤖 **« Humain » sur une place libre ne fige pas le salon** (corrigé le 2026-09-05) : l'hôte
  pouvait poser « Humain » sur une place que personne ne tient. `ready: false` était alors posé pour
  une confirmation que personne ne pouvait donner — « Lancer », qui exige toutes les places prêtes,
  devenait inerte **pour toujours** — et le retour en arrière était fermé, `canEditSlot` ne rendant la
  main que sur une place IA ou libre : le segment disparaissait de la ligne. Le salon n'avait plus
  d'autre issue que d'être quitté. « Humain » vaut désormais **place libre** (`Room.setSeatOccupancy`
  refuse `Human`) : la ligne garde « ⏳ Place libre », son segment reste actionnable **dans les deux
  sens**, et « Lancer » n'attend plus que la confirmation de l'hôte.
- 🤖 **Une place libre garde une équipe** (même correctif) : c'est elle qui joue si personne ne vient,
  `composeStartSeats` la rendant en IA au `start`, et c'est déjà ce que `canEditSlot` suppose en la
  rendant composable. Sans ça, « Humain » vidait la ligne — trois widgets de la même ligne se
  contredisaient (segment « Humain », carte vide, badge « Place libre »), « Lancer » s'éteignait sur
  la règle locale « aucun camp vide » pour une place que le salon déclare prête, et le salon partait
  en combat avec l'équipe **périmée** que l'écran venait de montrer comme retirée (`announceSelection`
  sortant sur une équipe nulle). Vérifié par `data-state="ephemeral"` sur son bouton d'équipe.
...
| `combat/mechanics-field-global.spec.ts` | §5.29 famille Field global (plan 145) : les 3 zones diamant Self (Gravité / Zone Étrange / Zone Magique) posées sur la case du lanceur (2,3) → journal « déploie <label> (5 tours) » + quad de scène par kind (`field_terrain_<couleur>_2_3`, une couleur par kind) ; Vent Arrière (`tailwind`, GroundTarget portée 1) ciblé au nord (2,2) → journal « lève le Vent Arrière vers le Nord » + HUD flèche `tailwind-hud` (libellé « Vent Arrière ») ; Gravité verrouille les moves aériens → Pied Voltige (`disabledUnderGravity`) `data-enabled="false"` au menu Attaque une fois la zone posée, `data-enabled="true"` en témoin. Zone Magique neutralise l'objet tenu d'un mon dans la zone (plan 171) : le Mackogneur pose Zone Magique (2,3) puis Séisme (Sol) → le Ronflex adjacent porteur du Ballon (`air-balloon`) perd son immunité Sol → « Ronflex perd N PV ! » ; témoin sans zone → immunisé (aucun dégât). Clouage des Volants + immunité Sol, précision ×5/3, swap Déf/DéfSpé, les 15 autres effets d'objet routés (Veste de Combat, Herbe Mentale, verrou Choice, Dé Pipé, Grosse Racine, Lunettes Filtre…), ctGain ×1.5 aligné au vent, décompte/expiration/horloge fantôme = unit/integration core (`moves/{gravity,wonder-room,magic-room,tailwind}.test.ts`, `effective-held-item`, `field-global-system.test.ts`, `tailwind-system.test.ts`) → 👁 |
...
| `dom/online-lobby.spec.ts` | §6.8 jeu en ligne (plan 199) — **deux** scénarios. **(1) À deux contextes de navigateur** : l'hôte choisit son format, crée, passe par le terrain, et voit naître le code ; le sélecteur de format a disparu de la salle d'attente ; l'invité **saisit le code au clavier dans la roue** (couvre la collision `KeyS`/`KeyD` avec les touches de mouvement) et rejoint sans avoir choisi de carte ; l'hôte voit la ligne passer en « Joueur distant » et « Lancer » reste inerte ; l'invité compose SA ligne (la deuxième — sa place, pas la première) puis confirme ; l'hôte lance et **les deux scènes deviennent prêtes**, ce qui prouve le lancement accusé. **(2) À un seul contexte, l'hôte seul dans son salon** (partie à 4 places, donc des lignes libres à basculer) : presser « Humain » sur une place que personne ne tient la laisse « ⏳ Place libre », lui garde un segment **actionnable dans les deux sens** (IA ↔ place libre), lui laisse une **équipe** (`data-state="ephemeral"` — celle qui joue si personne ne vient), et ne tue pas « Lancer » — la régression du 2026-09-05, où `ready: false` était posé pour une confirmation que personne ne pouvait donner, où la ligne perdait son segment, et où elle perdait aussi son équipe. Pas de second pair, donc pas de négociation WebRTC à payer. 🔴 **Annuaire LOCAL** (paquet `peer`, second `webServer`, `?peerPort=` verrouillé sur DEV/E2E) et **STUN/TURN désactivés** : la suite ne demande rien à Internet. Tout ce qui se teste sans réseau — allocation de places, maillage, départs, lancement annulé — est en **intégration** (`packages/network`), sur le canal en mémoire |
...
- [x] **Aide visuelle des gestes attendus** (suite du Lot 1 du plan-cadre 173) : glyphe de geste dans
      ...
      timeline (spec ensuite renommée `compass-and-legend.spec.ts` au plan 185, § ci-dessus).
      **Débloqué par un ajout au hook** : `meshScreenBox(name)` projette la boîte englobante d'un mesh
      à l'écran (lecture seule, strippé du build prod), seul moyen de situer — et de cliquer — un mesh
      épinglé à un coin d'écran, dont la position monde est recalculée depuis la caméra à chaque
      frame. *Restaient 👁 alors : les deux dessins (souris/main, flèche circulaire) et le sens de
      rotation tel que l'œil le lit — le glyphe de rotation en mesh Babylon est depuis **supprimé**
      (plan 185, remplacé par la légende de contrôles en DOM).*
- [x] **§4.18 Légende de contrôles près de la boussole** (plan 185) : glyphe « ça se clique » à
      droite de la boussole + lignes rotation/zoom en dessous, ancrées sur la même mesure que le
      renderer (`chrome-insets.ts`, gagne un `subscribe`), immobile quand la timeline perd son entrée
      active, suit la source d'entrée (souris/doigt/clavier/manette), zone tapable de la boussole
      redevenue un simple carré (`compass-and-legend.spec`, 6 cas). Étiquette de touche selon la
      disposition clavier (`getLayoutMap` + repli langue) en unit (`key-legend.test.ts`, 6 cas). Feuille
      de curseurs Kenney intégrée en variante masque pour le zoom/pinch, réutilisée par le glyphe de la
      ligne d'instruction (`input-prompt-glyph.spec`, +2 cas). *Restent 👁 : dessin des glyphes,
      lecture au doigt (ligne rotation masquée, boussole qui tourne la vue au tap) — téléphone réel.*
