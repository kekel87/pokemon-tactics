...
4. **Vérifier l'attachement aux Pokemon** : l'`abilityId` est dérivé automatiquement de l'espèce
   (`ability1` de `reference/pokemon.json`, cf. `packages/data/src/loaders/load-pokemon.ts`). Seules les
   entrées `custom` de `packages/data/src/playable/playable-pokemon.ts` le posent à la main (ex. Métamorph)
