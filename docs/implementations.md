...
| 132 | ditto | Métamorph | Normal | ✓ | imposter | ⚠️ Faint abs. | Roster `custom` (plan 157) : movepool `[transform]`, stats 48 partout, sprite 0132 |
...
| Boul'Armure | defense-curl | Normal | Statut | — | — | 40 | self | +1 Déf. Pose `usedDefenseCurl` (persistant, reset au KO) : double la puissance de Roulade/Ball'Glace pour le reste du combat. `EffectKind.MarkDefenseCurl`. |
...
| Roulade | rollout | Roche | Phys | 30 (→480 snowball, →960 si Boul'Armure) | 90 | 20 | dash r2 (→r5 snowball) | **Snowball** : portée +1/cast consécutif (cap r5), puissance ×2/cast (cap 480). Reset si autre move. `DynamicPowerKind.RolloutStreak`. **×2 supplémentaire** si Boul'Armure (`defense-curl`) a été utilisée avant dans le combat (`usedDefenseCurl`) — cumulatif avec le snowball. `rolloutEffectivePower`. |
...
| Faux-Chage | false-swipe | Normal | Phys | 40 | 100 | 40 | single r1 | Ne peut jamais mettre K.O. (cible garde ≥1 PV). Flag `cannotKo`. |
| Croc Fatal | super-fang | Normal | Phys | — | 90 | 10 | single r1 | Dégâts fixes = ⌊PV actuels cible / 2⌋ (min 1), ignore le typechart. `EffectKind.HalveTargetHp`. |
| Ruse | feint | Normal | Phys | 30 | 100 | 10 | single r1 | Ignore Protection / Détection (`bypassProtect`). |
| Anti-Air | smack-down | Roche | Phys | 50 | 100 | 15 | single r1-3 (distance) | Cloue un Volant au sol via `smackedDown` = même prédicat que Gravité (`isEffectivelyGrounded`) : terrain/lave/poison, hazards, mouvement, atterrissage visuel. Tick terrain immédiat au grounding. Annule Vol/Rebond en cours. Permanent. `EffectKind.SmackDown`. |
| Poursuite | pursuit | Ténèbres | Phys | 40 | 100 | 20 | single r1 | ×2 si le coup atteint le dos de la cible (stacke avec le bonus de dos → 2.3×). Flag `pursuitBackstab`. |
| Corps Perdu | vital-throw | Combat | Phys | 70 | — | 10 | single r1 | Ne rate jamais (`bypassAccuracy`). Priorité -1 canon sans objet (CT seul). |
...
| Fuite | run-away | Rattata, Rattatac, Ponyta, Galopa, Doduo, Dodrio, Évoli | No-op documenté (plan 158) — pas de fuite/combat sauvage dans ce jeu. Marker handler vide : évite que ces 7 mons résolvent un talent `undefined` en combat. |
| Ramassage | pickup | Miaouss | No-op documenté (plan 158) — pas de ramassage hors-combat. Marker handler vide. |
...
| Bandeau | focus-band | 10% de survie à 1 PV sur coup fatal, depuis n'importe quels PV, non consommé (roll PRNG déterministe, `survivesLethalHitChance`) |
| Griffe Rasoir | razor-claw | +1 cran critique permanent, non consommé (`onCritStageBoost`) |
| Poing Chance | lucky-punch | +2 crans critiques, Leveinard uniquement (gate espèce) |
| Dé Pipé | loaded-dice | Moves multi-frappes toujours au maximum de coups (`maximizesMultiHit`) |
| Cape Obscure | covert-cloak | Supprime les effets secondaires entrants du porteur (`suppressesIncomingSecondary`) |
| Pierrallégée | float-stone | Poids du porteur ÷2 (lu par `effectiveWeight`) |
| Bande Étreinte | binding-band | ×2 dégâts/tour des pièges du porteur (`doublesTrapDamage`) |
| Accro Griffe | grip-claw | Durée des pièges partiels du porteur forcée au maximum (`maximizesTrapDuration`) |
| Carapace Mue | shed-shell | Immunité totale au piégeage du porteur (`immuneToTrapping`) |
| Poudre Métal | metal-powder | Déf + Déf.Spé ×1.5, Métamorph non transformé uniquement |
| Poudre Vite | quick-powder | Vitesse ×2, Métamorph non transformé uniquement (lu par `effectiveBaseSpeed`) |
