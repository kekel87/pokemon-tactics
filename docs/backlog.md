...
### Style dupliqué entre couche DOM et couche 3D Babylon (2026-05-19, MAJ 2026-07-19 — code mort purgé)
...
- **Code mort purgé (2026-07-19)** : les constantes de positionnement de l'ère canvas (`INFO_PANEL_*`, `ACTION_MENU_*`, `TIMELINE_*`, `BATTLE_LOG_*`, `DEPTH_BATTLE_LOG`) de `packages/app/src/constants.ts`, importées nulle part, ont été supprimées.
- **Fix restant** (priorité basse, maintenance) : centraliser les tokens partagés (couleurs/police) en une source unique exposée aux deux couches.
