...
- ~~**Familles restantes sans heuristique fine (Phase 3, différé)**~~ **résolu (plan 161, 2026-07-14 ; dernier reliquat item-interaction utilitaires — Tour de Magie/Passe-Passe/Gaz Corrosif — clos par la décision #714, 2026-07-23)** : Stat/state manip, item-interaction utilitaires, pièges purs position-linked (Barrage/Regard Noir), crit-manip Batch A (Puissance/Affilage/Cri Draconique/Yama Arashi/Dark Lariat), Cognobidon/Attraction sont désormais toutes valorisées dans `action-scorer.ts`. **Seul reliquat IA restant** : objets tenus passifs légers (plan 158) — won't-fix documenté (décision #714), toujours-actifs et ne conditionnant aucune décision IA, un bonus dédié serait du bruit. Consigné dans le graphe, entités `agenda`.
