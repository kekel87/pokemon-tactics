...
- **Statut** : `draft` — recherche faite, rien d'implémenté
- **Date** : 2026-08-06
...
- Pas de Fullscreen API sur iPhone (iPad seulement) → pas de masquage de barre par JS.
- Pas d'API Screen Orientation → aucun verrouillage d'orientation.
- Pas de `beforeinstallprompt` → installation à l'écran d'accueil forcément manuelle.
- Seul levier plein écran/orientation sur iPhone : manifeste PWA + installation manuelle. Sinon, l'overlay CSS d'obstruction reste la seule protection.
