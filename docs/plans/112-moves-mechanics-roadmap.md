...
| Implémentés | **348** | clés `tacticalOverrides` (`isMoveImplemented`) |
| Implémentés ∈ pool | 338 | ~10 moves signature ajoutés hors pool roster |
| **Restants in-scope** | **155** | `493 − 338` |
...
#### B3 · Dégâts conditionnels — 17 moves
→ **Extension du moteur `dynamicPower` (plan 109)** : puissance/effet selon état du tour. Gros volume, peu de neuf.
