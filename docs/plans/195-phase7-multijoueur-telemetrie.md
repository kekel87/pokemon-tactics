...
> 2026-09-04** (plan 199 — transport, salon, lancement accusé) ; **B2 ✅ livré et validé à la main le
...
- **B2 — Combat en réseau.** ✅ **LIVRÉ et validé à la main le 2026-09-08 (plan 201).** Seed partagé, échange des sélections d'équipe, tour distant greffé sur `humanPlayerIds`, validation de chaque action reçue contre `getLegalActions()` avec le barème 1er/2e/3e **consécutifs** (journal → avertissement → forfait). Forfait dans le core (`BattleEngine.forfeit`, patron de Lien du Destin). Critère atteint : un 1v1 complet de bout en bout, jusqu'à l'écran de victoire, sur le service public de PeerJS. Réseau restreint au **1v1** : le garde-fou d'index (détecteur de désync du pauvre) suppose un canal ordonné, vrai par connexion mais pas entre connexions distinctes — l'écran `lobby` annonce désormais le format au lieu de l'offrir.
- **B3 — Robustesse.** ✅ **LIVRÉ le 2026-09-09 — plan détaillé : `202-lot-b3-robustesse-multijoueur.md`.** Chronomètre **local auto-déclarant** dont le timeout produit une *action* et non un message réseau (#864) ; chien de garde de connexion **distinct** du chrono, à `chrono + marge` (#865) ; reconnexion par le chemin du plan 181, hôte compris (`scheduleHostRedial`, décisions #957, #960) ; abandon volontaire propagé aux deux pairs. Critère atteint : couper le réseau d'un pair et revenir. **Trois valeurs fixées en cadrant** : chrono à **60 s** et non 45 s (une seule fenêtre par tour doit couvrir déplacement + attaque + visée + orientation ; `enterActionMenu()` est rappelé à chaque étape, donc le redémarrer permettrait de geler la partie en annulant en boucle), chien de garde à **75 s** puis, à la deuxième chute de la même place, **30 s** (`BATTLE_GRACE_SHORT_MS`, décision #961 — corrige un 10 s hérité du salon qui donnait moins de grâce à une fermeture d'onglet propre qu'à un câble arraché), et le repli du timeout est **`EndTurn`** et non « Attendre », illégal après un déplacement déjà validé. La note de ce plan sur `CT_WAIT` (le coût le plus bas de la table, donc le timeout est le plus rentable en tempo) est reprise et assumée. **Quatre bugs d'asymétrie** trouvés en cours de lot (recette humaine et revue de code), tous la même famille : une relation testée dans un seul sens.
- **B4 — Détection de désync.** ✅ **CODÉ le 2026-09-10 — plan détaillé : `203-lot-b4-detection-desync.md`.** Restent l'e2e à deux contextes, le cahier de recette et la recette humaine. **`state-checksum.ts` vit dans `packages/core/src/battle/`, pas dans `packages/network`** — amende ce cadrage : module pur qui connaît la forme de l'état, sérialisation canonique (ordre des clés, `Map` triée, tableaux préservés, flottants quantifiés) puis hash FNV-1a comparé **à chaque action**, pas tous les N tours. ⚠️ **Amende aussi le repli annoncé ici** : à l'écart constaté, **constat et forfait, sans reconstruction depuis le replay** — en 1v1 personne ne peut dire qui s'est écarté (#943), réparer voudrait dire adopter la version d'en face sans preuve. Ce point restait « le plus sous-estimé du document d'avril » sur un axe qui s'est confirmé : la sérialisation canonique, à ne pas traiter comme un `JSON.stringify`.
