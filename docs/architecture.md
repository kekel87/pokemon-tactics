...
- **DOM** (`packages/ui-dom/src/info-panel.ts` + `styles/info-panel.css`) : `hideExactHp` → `hpNumbers` vide, `hpPct` sans parenthèses et `data-hp-only` (hérite taille/couleur primaires) ; ARIA suit ce qui est visible (`aria-valuemax="100"`, `aria-valuenow=<pct>`) pour ne pas annoncer aux lecteurs d'écran une info masquée à l'écran. Placeholders `itemUnknown`/`abilityUnknown` → `data-unknown="1"`, `.ip-item-glyph` (carré pointillé + `?`, **dessiné en CSS**, aucun asset requis) remplace l'icône officielle, même encombrement pour que la ligne ne saute pas à la révélation.
