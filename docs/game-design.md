...
- Revival : **Vœu Soin** (`healing-wish`, plan 147, décision #604), réinventé en move de revive ciblé (tuile r3) : ressuscite un allié KO à **50% PV max** ou soigne un vivant à **100%**, self-KO du lanceur en échange — premier et seul move de revive du jeu.
  ...
  🔴 **N'importe quelle cible K.O. à portée, sauf le dernier mort d'un camp entièrement à terre — en ligne.** L'option de règle `reviveDefeatedCamps` (plan 210), fausse en ligne, refuse cette cible-là : le move échoue, et le lanceur meurt quand même (sacrifice payé d'avance). **En local, l'option est vraie : le même geste réanime**, réponse de l'humain au cadrage (« en multi local, ça peut être marrant »)
...
- **À 3 camps et plus (FFA)** : un camp perd quand son dernier Pokemon tombe — le moteur l'annonce (événement `PlayerEliminated`, plan 210), y compris le **dernier** camp à tomber, juste avant le verdict — mais la partie **continue** tant qu'il reste au moins deux camps vivants : perdre son camp n'est pas la fin de la partie pour les autres. **En ligne**, le joueur éliminé voit un dialogue à deux issues : « Retour au menu » ou « Continuer à regarder » (mode spectateur, caméra libre). **En local** (hot-seat), une ligne de journal suffit — l'écran est partagé, personne n'y reçoit de choix qui engagerait les autres. Détail : `docs/plans/210-joueur-elimine-et-mode-spectateur.md`
