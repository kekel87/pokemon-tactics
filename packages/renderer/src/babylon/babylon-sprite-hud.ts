...
/** HP-bar canvas resolution (kept crisp via NEAREST); width derives from the world ratio. */
const BAR_TEXTURE_HEIGHT = 40;
const BAR_TEXTURE_WIDTH = Math.round(
  BAR_TEXTURE_HEIGHT * (BABYLON_HP_BAR_WIDTH / BABYLON_HP_BAR_HEIGHT),
);
/** Border thickness + corner radius drawn on the bar canvas (px). */
const BAR_BORDER_PX = 6;
const BAR_RADIUS_PX = BAR_TEXTURE_HEIGHT * 0.42;
...
/** A team-aura icon stacked to the left of the HP bar (mirrors the Phaser left-indicator stack). */
...
/** A predicted-damage overlay on a target's HP bar (confirm phase, plan 123 4d-4). */
...
export function createSpriteHud(scene: Scene): SpriteHud {
  ...
  function redrawBar(record: HudRecord): void {
    const context = record.barContext;
    const width = BAR_TEXTURE_WIDTH;
    const height = BAR_TEXTURE_HEIGHT;
    context.clearRect(0, 0, width, height);

    // Border ring: full rounded rect in the border colour, then the dark track inset.
    roundRectPath(context, 0, 0, width, height, BAR_RADIUS_PX);
    context.fillStyle = BAR_BORDER_CSS;
    context.fill();

    const inset = BAR_BORDER_PX;
    const innerLeft = inset;
    const innerTop = inset;
    const innerWidth = width - inset * 2;
    const innerHeight = height - inset * 2;
    const innerRadius = Math.max(0, BAR_RADIUS_PX - inset);
    roundRectPath(context, innerLeft, innerTop, innerWidth, innerHeight, innerRadius);
    context.fillStyle = BAR_BG_CSS;
    context.fill();

    const ratio = record.maxHp > 0 ? Math.max(0, Math.min(1, record.currentHp / record.maxHp)) : 0;
    const fillWidth = innerWidth * ratio;
    if (fillWidth > 0) {
      roundRectPath(context, innerLeft, innerTop, fillWidth, innerHeight, innerRadius);
      context.fillStyle = record.teamColorCss;
      context.fill();
    }
    ...
    if (estimate && record.maxHp > 0 && !estimate.immune) {
      const max = record.maxHp;
      const currentRatio = Math.max(0, Math.min(1, record.currentHp / max));
      const afterMaxRatio = Math.max(0, (record.currentHp - estimate.max) / max);
      const afterMinRatio = Math.max(0, (record.currentHp - estimate.min) / max);
      context.save();
      roundRectPath(context, innerLeft, innerTop, innerWidth, innerHeight, innerRadius);
      context.clip();
      const possibleWidth = (afterMinRatio - afterMaxRatio) * innerWidth;
      if (possibleWidth > 0) {
        context.fillStyle = `rgba(0,0,0,${DAMAGE_ESTIMATE_ALPHA_POSSIBLE})`;
        context.fillRect(
      ...
      if (guaranteedWidth > 0) {
        context.fillStyle = `rgba(0,0,0,${DAMAGE_ESTIMATE_ALPHA_GUARANTEED})`;
        context.fillRect(
