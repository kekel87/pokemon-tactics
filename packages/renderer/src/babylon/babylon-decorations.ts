...
export function createDecorations(
  ...
  // Cells covered by a rock/tree footprint — tall-grass skips these. Their
  // rendered top is recorded in `place` (when the texture size is known) so the
  // cursor can rest on it and flyers can rise onto it.
  ...
  for (let y = 0; y < map.height; y++) {
    for (let x = 0; x < map.width; x++) {
      if (map.tiles[y]?.[x]?.terrain === TerrainType.TallGrass && !occupied.has(`${x},${y}`)) {
        place(DecorationKind.TallGrass, x, y, 1, 1);
      }
    }
  }
