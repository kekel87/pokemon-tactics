...
import type { HighlightKind } from "../enums/highlight-kind.js";
import {
...
} from "../game/movement-animation.js";
...
import {
  type AuraIndicatorSpec,
  createSpriteHud,
  type DamageEstimateView,
  type SpriteHudHandle,
} from "./babylon-sprite-hud.js";
...
/** A Pokemon placed on the battlefield at a grid cell, optionally facing a direction. */
export interface CombatSceneSpawn {
  pokemonId: string;
  spawn: { x: number; y: number };
  /** World facing in radians (default 0 = South-ish). 8-way display is muxed against the fixed camera azimuth. */
  facing?: number;
...
/** A Pokemon added after scene creation (placement phase). Remove via `removePokemon`. */
export interface CombatPokemonHandle {
  /** Re-orient the billboard toward a core grid direction. */
  setFacing(direction: Direction): void;
  /**
   * Teleport the billboard to another tile (snap — used for re-sync + non-walk
   * relocations like teleport / Hit&Run retreat; knockback + ice-slide glide).
   */
  moveTo(tile: { x: number; y: number }): void;
  /**
   * Glide the billboard along a path of tiles (plan 122 4c-2, per-step animation
   * parity 4d-5): per-step facing + Walk/Hop chosen from each step's height delta,
   * flyers gliding over special terrain. Resolves once the last step lands.
   */
  moveAlongPath(
  ...
  /**
   * Face a direction and play a one-shot attack animation (plan 122 step 4c-3),
   * resolving when it ends. A safety cap resolves anyway if the animation is
   * absent or the scene is torn down mid-attack (never hangs the queue).
   */
  playAttack(direction: Direction, animationName: string): Promise<void>;
  /**
   * Glide to a destination tile without changing facing (knockback / ice-slide),
   * optionally playing the Hurt pose. Resolves on arrival.
   */
  impactGlide(tile: { x: number; y: number }, options?: { hurt?: boolean }): Promise<void>;
  /** Hurt pose + brief shake, then rest (knockback blocked). Resolves when it ends. */
  impactShake(): Promise<void>;
  /** Active-Pokémon breathing pulse (the current turn's actor). */
  setActive(active: boolean): void;
  /** Brief red damage flash (the only combat "juice" wired in the 7b minimal loop). */
  flashDamage(): void;
  /** Sustained pulse marking a locked attack target during the confirm phase (parity). */
  setPreviewFlash(active: boolean): void;
  /** Roll the sprite back and forth while confused (volatile), upright when cleared. */
  setConfusionWobble(active: boolean): void;
  /** Update the world HP bar fill (ratio = currentHp/maxHp). */
  updateHp(currentHp: number, maxHp: number): void;
  /** Show the major status icon over the sprite (first status effect), or clear it when null. */
  ...
  /** Replace the team-aura icons stacked left of the HP bar (empty clears). */
  setLeftIndicators(specs: readonly AuraIndicatorSpec[]): void;
  /** KO: dark tint + frozen Faint pose (also hides the HP bar + status icon). */
  setKnockedOut(knockedOut: boolean): void;
  /** Semi-invulnerable display (flying lift / underground hide). */
  setSemiInvulnerable(state: SemiInvulnerableDisplay): void;
  /** Play a one-shot animation by name (e.g. "Attack"); silently ignored if absent. */
  playOnce(animation: string): void;
  /** Clonage (substitute): show the dummy doll while the volatile is up, real sprite when it breaks. */
  setSubstitute(active: boolean): void;
  /** Show/hide the world HUD (HP bar + status + indicators), e.g. during direction selection. */
  setHudVisible(visible: boolean): void;
  /** Total ms of the Faint animation (current facing), to pace the KO beat on its real length. */
  koAnimationDurationMs(): number;
}

/** Callbacks for the in-scene placement direction picker (plan 120 step 6, décision #487). */
export interface DirectionPickerCallbacks {
  /** Hovered a different arrow — preview the orientation on the temp billboard. */
  onPreview: (direction: Direction) => void;
  /** Clicked an arrow — commit this facing. */
  onConfirm: (direction: Direction) => void;
  /** Escape or a click off the arrows — abort the facing choice. */
  onCancel: () => void;
}
...
export interface CombatScene {
  ...
  /** Replace the fill set of one highlight kind (move/attack/retreat/enemy). No-op until the map loads. */
  setTileHighlights(kind: HighlightKind, positions: readonly TileHighlightPosition[]): void;
  /** Replace the spawn-zone fills (per-zone colour/alpha; empty clears). No-op until the map loads. */
  setSpawnZoneHighlights(zones: readonly SpawnZoneHighlight[]): void;
  /** Replace the range outline (external edges of the set); `beneficial` paints it blue. */
  setTileOutline(positions: readonly TileHighlightPosition[], beneficial?: boolean): void;
  /** Replace the painted field-terrain ("Champs") zones (empty clears). No-op until the map loads. */
  setFieldTerrains(specs: readonly FieldTerrainSpec[]): void;
  /**
   * Float the given aura symbols over every tile of an aura caster's radius
   * (hover feedback; empty `cells` or `symbols` clears). No-op until the map loads.
   */
  setAuraGroundIcons(cells: readonly { x: number; y: number }[], symbols: readonly string[]): void;
  /** Clear all fill highlights + spawn zones + outline. */
  clearHighlights(): void;
  /** Add a Pokemon billboard on a tile (placement phase). Requires `ready` resolved. */
  addPokemon(entry: CombatSceneSpawn): CombatPokemonHandle;
  /** Remove a dynamically added Pokemon (placement undo/cancel). */
  removePokemon(handle: CombatPokemonHandle): void;
  /**
   * Open the in-scene direction picker on a tile's four neighbours (placement
   * facing choice). Returns a handle whose `dispose` closes it silently. Hover/
   * click/Escape are routed by the scene's own pointer handling.
   */
  showDirectionPicker(
  ...
  /** Called on each hover move with the tile under the pointer (or `null` off-grid). */
  onTileHover(handler: (pick: TilePick | null) => void): void;
  /** Called on a tile click (a left press released without panning). */
  onTileClick(handler: (pick: TilePick) => void): void;
  /** Glide the camera target onto a tile (centre on the active Pokémon each turn). */
  panCameraTo(tile: { x: number; y: number }): void;
  /**
   * Spawn a short drifting combat label (damage number, "Miss!", status…) over a
   * tile. Projected to screen once at spawn, then rises + fades via CSS. No-op
   * until the map loads.
   */
  spawnFloatingText(
    tile: { x: number; y: number },
