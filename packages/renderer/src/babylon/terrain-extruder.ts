...
import type { LoadedTiledMap, VisualTerrainGroup } from "./load-tiled-map.js";
 ...
 * half = 0.5), floored to avoid 0-height, then squashed by BABYLON_TILE_HEIGHT_SCALE
 * so the side walls aren't as tall as a full cube (the top diamond is unaffected —
...
export function tileBodyHeight(tileHeight: number): number {
  return Math.max(BABYLON_TILE_MIN_HEIGHT, tileHeight) * BABYLON_TILE_HEIGHT_SCALE;
}

/**
 * Grid (col, row) → world XZ, matching the 2D iso renderer's orientation:
 * gridX runs toward screen down-right, gridY toward down-left, tile (0,0) at top.
 * That requires mapping gridX onto world Z and gridY onto world X (a transpose vs
 * the naive mapping) — see `packages/renderer/src/grid/iso-math.ts` for the 2D convention.
 */
