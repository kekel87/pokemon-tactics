...
import { drawTextGlyphs, measureTextPlane } from "@pokemon-tactic/render-canvas2d";
...
export function createTextPlane(scene: Scene, options: TextPlaneOptions): TextPlane {
  ...
  const { textureWidth, textureHeight } = measureTextPlane(
    options.text,
    fontPx,
    FONT_FAMILY,
    BABYLON_HUD_TEXT_PADDING_PX,
    strokePx,
  );
