...
import { handleActAfterUser } from "./handlers/grid-targeting/handle-act-after-user";
import { handleDrawAttention } from "./handlers/grid-targeting/handle-draw-attention";
import { handleSwapAllyPositions } from "./handlers/grid-targeting/handle-swap-ally-positions";
...
import { handleSmackDown } from "./handlers/handle-smack-down";
...
import { handleSuperFang } from "./handlers/handle-super-fang";
...
export function processEffects(
  ...
  if (moveHasDamage) {
    ...
    for (const target of context.targets) {
      ...
      // Gravité / Anti-Air: a grounded defender loses its Flying-type immunity to Ground moves.
      const grounded = isEffectivelyGrounded(context.state, target);
