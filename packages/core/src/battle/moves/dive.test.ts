...
describe("dive", () => {
  it("T1 charge emits MoveCharging, sets Diving semi-invulnerable state, no damage", () => {
    ...
    expect(casterState?.semiInvulnerableState).toBe(SemiInvulnerableState.Diving);
    expect(casterState?.chargingMove?.moveId).toBe("dive");
    expect(casterState?.lockedMoveId).toBe("dive");
  ...
  it("T2 teleports to empty landing tile and deals damage to adjacent foe", () => {
    const attacker = MockPokemon.fresh(MockPokemon.base, {
      ...
      moveIds: ["dive"],
      currentPp: { dive: 9 },
      chargingMove: { moveId: "dive", targetPosition: { x: 0, y: 0 } },
      lockedMoveId: "dive",
