...
describe("scorching-sands", () => {
  it("deals damage to target within zone radius 1", () => {
    ...
    const user = MockPokemon.fresh(MockPokemon.base, {
      ...
      position: { x: 2, y: 2 },
      moveIds: ["scorching-sands"],
      currentPp: { "scorching-sands": 10 },
  ...
  it("applies burned status when proc triggers", () => {
    vi.spyOn(Math, "random").mockReturnValue(0);
    const user = MockPokemon.fresh(MockPokemon.base, {
      id: "user",
      playerId: PlayerId.Player1,
  ...
  it("does not apply burned status when proc does not trigger", () => {
