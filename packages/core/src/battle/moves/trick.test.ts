...
describe("trick", () => {
  it("swaps held items with the target", () => {
    ...
    expect(result.events.map((e) => e.type)).toContain(BattleEventType.ItemsSwapped);
    expect(state.pokemon.get(attacker.id)?.heldItemId).toBe(HeldItemId.Leftovers);
    expect(state.pokemon.get(defender.id)?.heldItemId).toBe(HeldItemId.ChoiceBand);
  });
});
