...
describe("calm-mind", () => {
  it("raises SpAttack and SpDefense by 1", () => {
    const user = MockPokemon.fresh(MockPokemon.base, {
      playerId: PlayerId.Player1,
      position: { x: 2, y: 2 },
      moveIds: ["calm-mind"],
      currentPp: { "calm-mind": 20 },
  ...
  it("emits StatChanged events for SpAttack and SpDefense", () => {
    const user = MockPokemon.fresh(MockPokemon.base, {
      playerId: PlayerId.Player1,
      position: { x: 2, y: 2 },
      moveIds: ["calm-mind"],
  ...
  it("does not affect physical stats", () => {
    const user = MockPokemon.fresh(MockPokemon.base, {
      playerId: PlayerId.Player1,
      position: { x: 2, y: 2 },
      moveIds: ["calm-mind"],
  ...
  it("stacks on second use", () => {
    ...
    const pokemon = state.pokemon.get(user.id)!;
    expect(pokemon.statStages[StatName.SpAttack]).toBe(2);
