...
describe("reflect", () => {
  it("posts a Reflect aura and emits AuraPosted", () => {
    const user = MockPokemon.fresh(MockPokemon.base, {
      playerId: PlayerId.Player1,
      position: { x: 2, y: 2 },
      moveIds: ["reflect"],
      currentPp: { reflect: 20 },
    ...
    expect(state.auras.some((a) => a.kind === AuraKind.Reflect)).toBe(true);
  });
  ...
  it("aura is associated with the caster", () => {
    ...
    const aura = state.auras.find((a) => a.kind === AuraKind.Reflect);
