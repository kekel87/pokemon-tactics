...
describe("steel-wing", () => {
  ...
  it("raises user defense by 1 stage when proc triggers", () => {
    vi.spyOn(Math, "random").mockReturnValue(0);
    const attacker = MockPokemon.fresh(MockPokemon.base, {
      id: "attacker",
      playerId: PlayerId.Player1,
  ...
  it("does not raise user defense when proc does not trigger", () => {
    ...
    expect(state.pokemon.get(attacker.id)?.statStages[StatName.Defense]).toBe(0);
    vi.restoreAllMocks();
  });
