...
describe("scary-face", () => {
  it("lowers target Speed by 2 stages on hit", () => {
    const attacker = MockPokemon.fresh(MockPokemon.base, {
      playerId: PlayerId.Player1,
      position: { x: 0, y: 0 },
      moveIds: ["scary-face"],
      currentPp: { "scary-face": 10 },
    ...
    expect(statEvent?.stat).toBe(StatName.Speed);
    expect(statEvent?.stages).toBe(-2);
    expect(statEvent?.targetId).toBe(defender.id);
    expect(state.pokemon.get(defender.id)?.statStages[StatName.Speed]).toBe(-2);
  });
