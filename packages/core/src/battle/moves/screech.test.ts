...
describe("screech", () => {
  it("lowers target Defense by 2 stages on hit", () => {
    const attacker = MockPokemon.fresh(MockPokemon.base, {
      playerId: PlayerId.Player1,
      position: { x: 0, y: 0 },
      moveIds: ["screech"],
      currentPp: { screech: 40 },
    ...
    expect(statEvent?.stat).toBe(StatName.Defense);
    expect(statEvent?.stages).toBe(-2);
    expect(statEvent?.targetId).toBe(defender.id);
    expect(state.pokemon.get(defender.id)?.statStages[StatName.Defense]).toBe(-2);
  });
  ...
  it("misses when accuracy roll fails", () => {
    ...
    expect(state.pokemon.get(defender.id)?.statStages[StatName.Defense]).toBe(0);
  });
