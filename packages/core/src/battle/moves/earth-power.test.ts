...
describe("earth-power", () => {
  ...
  it("lowers target SpDefense by 1 stage when proc triggers", () => {
    vi.spyOn(Math, "random").mockReturnValue(0);
    const attacker = MockPokemon.fresh(MockPokemon.base, {
      id: "attacker",
      playerId: PlayerId.Player1,
    ...
    expect(state.pokemon.get(defender.id)?.statStages[StatName.SpDefense]).toBe(-1);
    vi.restoreAllMocks();
  });
