...
describe("twister", () => {
  ...
  it("applies flinch volatile when proc triggers", () => {
    vi.spyOn(Math, "random").mockReturnValue(0);
    const attacker = MockPokemon.fresh(MockPokemon.base, {
      playerId: PlayerId.Player1,
      position: { x: 2, y: 2 },
    ...
    expect(result.success).toBe(true);
    expect(
      state.pokemon.get(foe.id)?.volatileStatuses.some((v) => v.type === StatusType.Flinch),
    ).toBe(true);
    vi.restoreAllMocks();
  });

  it("does not apply flinch when proc does not trigger", () => {
