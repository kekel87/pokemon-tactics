...
describe("safeguard", () => {
  it("posts a Safeguard aura and emits AuraPosted", () => {
    const user = MockPokemon.fresh(MockPokemon.base, {
      playerId: PlayerId.Player1,
      position: { x: 2, y: 2 },
      moveIds: ["safeguard"],
      currentPp: { safeguard: 25 },
    ...
    expect(state.auras.some((a) => a.kind === AuraKind.Safeguard)).toBe(true);
  });
  ...
  it("aura is associated with the caster", () => {
    ...
    const aura = state.auras.find((a) => a.kind === AuraKind.Safeguard);
