...
describe("dragon-dance", () => {
  it("raises Attack and Speed by 1 stage", () => {
    const user = MockPokemon.fresh(MockPokemon.base, {
      playerId: PlayerId.Player1,
      position: { x: 2, y: 2 },
      moveIds: ["dragon-dance"],
    ...
    expect(pokemon.statStages[StatName.Speed]).toBe(1);
  });

  it("emits StatChanged events for Attack and Speed", () => {
    const user = MockPokemon.fresh(MockPokemon.base, {
      playerId: PlayerId.Player1,
      position: { x: 2, y: 2 },
      moveIds: ["dragon-dance"],
