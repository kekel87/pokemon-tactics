...
describe("metal-sound", () => {
  it("lowers target Sp. Defense by 2 stages on hit", () => {
    const attacker = MockPokemon.fresh(MockPokemon.base, {
      playerId: PlayerId.Player1,
      position: { x: 0, y: 0 },
      moveIds: ["metal-sound"],
      currentPp: { "metal-sound": 40 },
    ...
    expect(statEvent?.stat).toBe(StatName.SpDefense);
    expect(statEvent?.stages).toBe(-2);
    expect(statEvent?.targetId).toBe(defender.id);
    expect(state.pokemon.get(defender.id)?.statStages[StatName.SpDefense]).toBe(-2);
  });
  ...
  it("misses when accuracy roll fails", () => {
    ...
    expect(result.events.map((e) => e.type)).not.toContain(BattleEventType.StatChanged);
    expect(state.pokemon.get(defender.id)?.statStages[StatName.SpDefense]).toBe(0);
  });
