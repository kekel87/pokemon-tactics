...
function psyshockDamage(defenderStats: typeof BASE_STATS): number {
...
describe("psyshock", () => {
  ...
  it("deals less damage to a high-Defense target despite low SpDef (uses physical defense)", () => {
    const highDefLowSpDef = psyshockDamage({ ...BASE_STATS, defense: 200, spDefense: 30 });
    const lowDefHighSpDef = psyshockDamage({ ...BASE_STATS, defense: 30, spDefense: 200 });

    expect(highDefLowSpDef).toBeLessThan(lowDefHighSpDef);
  });

  it("deals more damage when the target has low physical Defense, regardless of SpDef", () => {
    const baseline = psyshockDamage(BASE_STATS);
    const highSpDefOnly = psyshockDamage({ ...BASE_STATS, spDefense: 200 });
