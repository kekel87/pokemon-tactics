...
describe("brine", () => {
  it("deals damage to a target in range", () => {
    expect(damageTo(fire(2, 100).events, "defender")).toBeGreaterThan(0);
  });
  ...
  it("roughly doubles against a target at or below half HP", () => {
    const full = damageTo(fire(2, 100).events, "defender");
    const half = damageTo(fire(2, 50).events, "defender");
    expect(half).toBeGreaterThanOrEqual(full * 1.8);
    expect(half).toBeLessThanOrEqual(full * 2.2);
  });
});
