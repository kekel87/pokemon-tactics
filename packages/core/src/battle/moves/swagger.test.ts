...
describe("swagger", () => {
  it("raises target Attack by 2 stages when accuracy hits", () => {
    ...
    expect(state.pokemon.get(foe.id)?.statStages[StatName.Attack]).toBe(2);
    vi.restoreAllMocks();
  });

  it("confuses the target when proc triggers", () => {
    vi.spyOn(Math, "random").mockReturnValue(0);
    const user = MockPokemon.fresh(MockPokemon.base, {
      id: "user",
      playerId: PlayerId.Player1,
