...
describe("draining-kiss", () => {
  it("deals damage to adjacent target", () => {
    const attacker = MockPokemon.fresh(MockPokemon.base, {
      id: "attacker",
      playerId: PlayerId.Player1,
      position: { x: 0, y: 0 },
      moveIds: ["draining-kiss"],
      currentPp: { "draining-kiss": 10 },
  ...
  it("heals attacker for 75% of damage dealt", () => {
    const attacker = MockPokemon.fresh(MockPokemon.base, {
      id: "attacker",
      playerId: PlayerId.Player1,
      position: { x: 0, y: 0 },
    ...
    expect(state.pokemon.get(attacker.id)?.currentHp).toBeGreaterThan(hpBefore);
  });
