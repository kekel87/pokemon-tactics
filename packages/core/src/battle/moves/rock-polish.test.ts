...
describe("rock-polish", () => {
  it("raises Speed by 2 stages", () => {
    const user = MockPokemon.fresh(MockPokemon.base, {
      playerId: PlayerId.Player1,
      position: { x: 2, y: 2 },
      moveIds: ["rock-polish"],
      currentPp: { "rock-polish": 20 },
    ...
    expect(pokemon.statStages[StatName.Speed]).toBe(2);
  });

  it("emits a StatChanged event for Speed", () => {
    const user = MockPokemon.fresh(MockPokemon.base, {
      playerId: PlayerId.Player1,
      position: { x: 2, y: 2 },
      moveIds: ["rock-polish"],
  ...
  it("does not affect other stats", () => {
    ...
    expect(pokemon.statStages[StatName.Defense]).toBe(0);
  ...
  it("stacks on second use", () => {
    ...
    const pokemon = state.pokemon.get(user.id)!;
    expect(pokemon.statStages[StatName.Speed]).toBe(4);
  });
});
