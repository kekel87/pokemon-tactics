...
/**
 * Panneau des contrôles (plan 186, extrait de son écran par le plan 187).
 *
 * Deux rôles pour un seul panneau : réassigner, et **lister**. La majorité des joueurs ne changeront
 ...
 *
 * Monté par l'écran `controls` de la FSM **et** par le menu de combat (plan 187), sans quoi changer
 * une touche en pleine partie demanderait de quitter le combat — donc de le perdre.
 */
...
export function createControlsPanel(options: PanelOptions): Panel {
  const { onBack, embedded = false } = options;
  ...
  // Un seul rendu, appelé par la fabrique : contrairement aux réglages (que le changement de langue
  // reconstruit), ce panneau n'a aucun chemin de re-rendu — un remapping rafraîchit les cases en
  // place via `refreshAll()`.
  const render = (): void => {
    root = el("div", "mn-screen ct-screen");
    if (embedded) {
      root.classList.add(EMBEDDED_PANEL_CLASS);
    }
    ...
    const back = menuButton(t("controls.back"), onBack);
  ...
  return {
    get element(): HTMLElement {
      if (root === null) {
        throw new Error("controls panel used before render");
      }
      return root;
    },
    dispose() {
