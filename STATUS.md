...
> MAJ : 2026-05-19 (Phase 4 — plan 087 DONE — refactor `roster-poc.ts` → `playable-pokemon.ts` + dérivation `PokemonDefinition.movepool` learnset ∩ implémenté + fallback OP sets)
...
Prochain : bugs playtest (MapSelect noir 1v1, caméra 12v1) en sessions dédiées. Méga-évolutions → Phase 9.
