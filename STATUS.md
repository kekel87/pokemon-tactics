...
> MAJ : 2026-05-18 (Phase 4 — plan 085 polish DONE — refonte CSS modulaire 12 fichiers + Modal natif `<dialog>` + HTML sémantique + bug padding global + i18n noms FR Pokemon + nouvelles rules css.md/html.md)
...
**Phase 4 en cours.** Plans 069–085 terminés (+ polish CSS/Modal/i18n). Roster Gen 1 complet à 81 Pokemon. OP Sets : 160/160 `full` (100%). Météo complète. Team Builder UI livré et poli.

Prochain : **Plan 086 — refonte `TeamSelectScene`** (consomme équipes saved localStorage, sous-pick N mons selon format pour placement). Puis : post-086 refactor `roster-poc.ts` → `playable-pokemon.ts`. Méga-évolutions → Phase 9.
