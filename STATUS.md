...
> MAJ : 2026-05-19 (Phase 4 — plan 086 DONE — refonte `TeamSelectScene` + sous-pick au placement + extension `PlacementPhase` core + 6 composants UI DOM team-select + CSS team-select.css + i18n)
...
**Phase 4 en cours.** Plans 069–086 terminés. Roster Gen 1 complet à 81 Pokemon. OP Sets : 160/160 `full` (100%). Météo complète. Team Builder UI livré + TeamSelectScene refonte.

Prochain : refactor `roster-poc.ts` → `playable-pokemon.ts` (plan post-086 séparé). Méga-évolutions → Phase 9.
