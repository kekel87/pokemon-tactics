...
### Format standard
Lire tous les fichiers dans `.vibe/agents/` et `.vibe/skills/` et vérifier :

```yaml
---
name: kebab-case              # obligatoire, unique
description: ...              # obligatoire, claire et actionable
model: devstral-2             # modèle utilisé
---
```
...
- Les modèles sont adaptés : `devstral-2` pour les tâches standards
...
- Vérifier que `docs/architecture.md` et `docs/methodology.md` listent bien tous les agents
...
Vérifier la cohérence entre le tableau de `docs/methodology.md` (section "Orchestration des agents") et les agents réels :
...
| **Superviseur fantôme** | On compte sur Claude Code principal pour orchestrer mais les règles sont floues | Des agents qui devraient être chaînés mais ne le sont que via docs/methodology.md |
...
- Y a-t-il des interactions entre agents qui pourraient être formalisées dans les prompts plutôt que dans docs/methodology.md seul ?
...
## Orchestration
- Tableau docs/methodology.md ↔ agents réels : ✅ cohérent | ❌ décalages : ...
